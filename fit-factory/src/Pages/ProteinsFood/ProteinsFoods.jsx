...
const ProteinsFoods = () => {
  ...
  const fetchData = (page, order, sort) => {
    return axios.get(
      `https://helpful-free-baroness.glitch.me/Protein-Foods?_limit=12&_page=${page}&_sort=${sort}&_order=${order}`
    );
  };
  ...
  useEffect(() => {
    fetchData(page, order, sort)
      .then((res) => {
        console.log("ProteinsFoodData: ", res.data);
        setData(res.data);
      })
      .catch((err) => {
